package com.example.mutlitplebackstackscompose
...
import com.example.mutlitplebackstackscompose.ui.theme.MutlitpleBackstacksComposeTheme
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            MutlitpleBackstacksComposeTheme {
                                    ...
                                    onClick = {
                                        rootNavController.navigate(item.title.lowercase()) {
                                            popUpTo(rootNavController.graph.findStartDestination().id) {
                                                saveState = true // only for tab switching
                                            }
...
fun HomeNavHost() {
    ...
    NavHost(navController = homeNavController, startDestination = "home1") {
        for (i in 1..10) {
            composable("home$i") {
                GenericScreen("Home $i", onNextClick = {
                    if (i < 10) {
...
fun ChatNavHost() {
    ...
    NavHost(navController = chatNavController, startDestination = "chat1") {
        for (i in 1..10) {
            composable("chat$i") {
                GenericScreen("Chat $i", onNextClick = {
                    if (i < 10) {
    ...
    NavHost(navController = settingsNavController, startDestination = "settings1") {
        for (i in 1..10) {
            composable("settings$i") {
                GenericScreen("Settings $i", onNextClick = {
                    if (i < 10) {
